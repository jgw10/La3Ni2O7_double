...
def create_Esite_matrix(VS, A, ed, ep, eo):
    ...
    for row_idx in range(dim):
        ...
        for num in Ni_num:
            if num[0] != 2:
                diag_el += A + abs(num[0] - 2) * A / 2.
            if num[1] != 2:
                diag_el += A + abs(num[1] - 2) * A / 2.
...
def create_interaction_matrix_d8(VS, d_state_idx, d_hole_idx, S_val, Sz_val, A):
    ...
    t1 = time.time()
    print(f"interaction_d8 time {(t1 - t0) // 60 // 60}h, {(t1 - t0) // 60 % 60}min, {(t1 - t0) % 60}s")
...
def create_interaction_matrix_po(VS, p_idx_pair, apz_idx_pair, Upp, Uoo):
    ...
    out = sps.coo_matrix((data, (row, col)), shape=(dim, dim))
    t1 = time.time()
    print(f"interaction_po time {(t1-t0)//60//60}h, {(t1-t0)//60%60}min, {(t1-t0)%60}s")
